...
def main():
    ...
    ###################################     # create Simulation
    simulation = Simulation(timesteps=timesteps, params=params, initial_water_map=three_cm_deep, initial_external_shade_map=shademap_00, init_species='capillifolium')
    #########################################################################################################
    ...
    # Save TIME SERIES
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots/Replication/Figure_6a_{species}_bands_water.pdf').expanduser()
    fig.savefig(str(fig_path_and_name))

    # Save HEATMAP HEIGHT
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots/Replication/Figure_6a_{species}_heatmap_height.pdf').expanduser()
    fig2.savefig(str(fig_path_and_name))

    # Save HEATMAP MASS
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots/Replication/Figure_6a_{species}_heatmap_mass.pdf').expanduser()
