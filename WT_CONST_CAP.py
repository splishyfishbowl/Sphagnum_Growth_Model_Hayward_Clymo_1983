...
def main():
    ...
    # change the values for the depth to -3 cm
    ...
    ###################################     # create Simulation
    simulation = Simulation(timesteps=timesteps, params=params, initial_water_map=three_cm_deep, initial_external_shade_map=shademap_00, init_species=species)
    #########################################################################################################
    ...
    # DUMMY RUN
    runs_height, runs_masses, shades, waters = simulation.run(constant_watermap=three_cm_deep, adapt_water_table_height_to_average_carpet_height=False)
    ...
    # plot bands
    _,_, species = stats.plot_means_and_bands_of_runs(fig=fig, ax=ax1, constant_watermap = three_cm_deep, simulation=simulation, scaling=True, number_of_runs=number_of_runs, adapt_water_table_height_to_average_carpet_height=False)
    ...
    # Save TIME SERIES
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots/Water_table_constant/WT_const_{species}_bands_water.pdf').expanduser()
    fig.savefig(str(fig_path_and_name))

    # Save HEATMAP HEIGHT
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots/Water_table_constant/WT_const_{species}_heatmap_height.pdf').expanduser()
    fig2.savefig(str(fig_path_and_name))

    # Save HEATMAP MASS
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots/Water_table_constant/WT_const_{species}_heatmap_mass.pdf').expanduser()
