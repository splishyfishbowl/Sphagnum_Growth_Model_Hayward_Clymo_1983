...
def main():
    ...
    # shade 0.0
    shademap_00 = Shademap(number_of_rows=params['number_of_rows'],
                           number_of_spots_per_row=params['number_of_spots_per_row'],
                           items = None)
    
    # change the external shade to 0.3 everywhere
    shademap_00.set_up(constant_value=0.0)

    
    ###################################     # create Simulation
    simulation = Simulation(timesteps=timesteps, params=params, initial_water_map=initial_wm, initial_external_shade_map=shademap_00, init_species=species)
    #########################################################################################################
    ...
    # DUMMY RUN
    runs_height, runs_masses, shades, waters = simulation.run(watermap_series=series_of_watermaps_with_heights_from_wtd_array, adapt_water_table_height_to_average_carpet_height=False)
    print(series_of_watermaps_with_heights_from_wtd_array[300].get_items())
    stats = Statistics(results_height=runs_height, results_mass=runs_masses, results_shade=shades, results_water_table_depth=waters, timesteps=timesteps, params=params, species=species)
    # stats.plot_average_height_masses(instant_plot=True)
    ###### Change SPECIES ######################----------->---------->-------------------------------------_>-------------------------------------------------------------------!
    stats = Statistics(results_height=runs_height, results_mass=runs_masses, results_shade=shades, results_water_table_depth=waters, timesteps=timesteps, params=params, species=species)

    fig = plt.figure(figsize=(10,7))
    gs1 = GridSpec(12, 8, left=0.1, right=0.90, wspace=0.05, figure=fig)
    ...
    # plot bands
    print(series_of_watermaps_with_heights_from_wtd_array[300].get_items())
    _,_, species = stats.plot_means_and_bands_of_runs(water_table_series=series_of_watermaps_with_heights_from_wtd_array, fig=fig, ax=ax1, simulation=simulation, scaling=True, number_of_runs=number_of_runs, adapt_water_table_height_to_average_carpet_height=False)
    
    ax1.set_ylim(bottom=-0.1,top=10.0)
    ax1.tick_params(
        axis='x',          
        which='both',      
        bottom=False,      
    ...
    # Save TIME SERIES
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots//Watertable_dynamical/WT_app_{species}_bands_water.pdf').expanduser()
    fig.savefig(str(fig_path_and_name))

    # Save HEATMAP HEIGHT
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots//Watertable_dynamical/WT_app_{species}_heatmap_height.pdf').expanduser()
    fig2.savefig(str(fig_path_and_name))

    # Save HEATMAP MASS
    fig_path_and_name = Path(f'~/GitHub/Moss_growth/.venv_Model_Sphag/HaywardAndClymo1983/Thesis_Tex_files/Plots//Watertable_dynamical/WT_app_{species}_heatmap_mass.pdf').expanduser()
